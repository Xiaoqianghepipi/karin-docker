...
CMD ["sh", "-lc", "set -e; if [ -f package.json ]; then exec pnpm app; fi; if [ ! -f /app/karin-project/package.json ]; then echo '[Karin] 初始化项目中...' && pnpm create karin -y; fi; if [ ! -f /app/karin-project/package.json ]; then echo '[Karin] 未找到 /app/karin-project/package.json'; exit 1; fi; if [ -d \"${EXTRA_FONT_DIR}\" ]; then fc-cache -f \"${EXTRA_FONT_DIR}\" >/dev/null 2>&1 || true; fi; for name in @karinjs plugins; do mount_dir=\"/app/$name\"; project_dir=\"/app/karin-project/$name\"; if [ -d \"$mount_dir\" ] && [ -z \"$(ls -A \"$mount_dir\" 2>/dev/null)\" ] && [ -d \"$project_dir\" ]; then for item in \"$project_dir\"/* \"$project_dir\"/.[!.]* \"$project_dir\"/..?*; do [ -e \"$item\" ] || continue; mv \"$item\" \"$mount_dir\"/; done; fi; if [ -d \"$project_dir\" ]; then rm -rf \"$project_dir\"; fi; ln -s \"$mount_dir\" \"$project_dir\"; done; if [ -f /app/.env ]; then rm -f /app/karin-project/.env; ln -s /app/.env /app/karin-project/.env; elif [ -f /app/karin-project/.env ]; then cp /app/karin-project/.env /app/.env; fi; cd /app/karin-project; exec pnpm app;"]
